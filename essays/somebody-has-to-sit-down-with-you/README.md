...
The numbers bear this out at industry scale. By mid-2026, the top six frontier models cluster within roughly 100 Elo of each other on LMArena's public leaderboard. MMLU scores sit in the 93–97 range. The tool isn't scarce anymore; it's commodified. And when Meta submitted an experimental Llama 4 Maverick to the Arena under conditions that produced a top ranking, Yann LeCun confirmed the results had been "fudged" — benchmark manipulation by a frontier lab, which is the AI equivalent of a forged permission slip. The credential system the industry uses to sort itself carries the same trust problems the rest of this essay is about.[^9]
...
[^9]: LMArena Elo scores and benchmark clustering from a Nestor verified-corpus store (185 pairs, 36 edges, 77 evidence records, August 2026) cross-referencing LMArena, BenchLM, Vellum, and MorphLLM data. On Meta's Llama 4 Maverick submission: Yann LeCun's acknowledgment that "results were fudged" reported via Slashdot and The Verge, confirmed by cherry-picked chat-optimized variant not matching the public release.
